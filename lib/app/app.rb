...
class ConnectorsWebApp < Sinatra::Base
  ...
  before do
    ...
    raise StandardError.new 'You need to set an API key in the config file' unless [:test, :development].include?(settings.environment) && settings.api_key == ConnectorsApp::DEFAULT_PASSWORD
